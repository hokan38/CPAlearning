...
def main():
    ...
    resolved_extra = []
    body_cursors = {}  # 過去問解答例(type qa)用: ページ本文下の余白カーソル
    for a, rects in resolved:
        ...
        try:
            if t == "highlight":
                ...
                # 問題が載っているページ下部の余白に解答例を直接記載。
                # 入らなければコメント帯へ退避。
                h_pg = page.rect.height
                y0 = body_cursors.get(a["page"])
                if y0 is None:
                    ymax = 0
                    for b in page.get_text("blocks", clip=pymupdf.Rect(0, 0, ow, h_pg)):
                        if b[1] > h_pg - 48:  # ページ番号フッター行は無視
                            continue
                        ymax = max(ymax, b[3])
                    for dr in page.get_drawings():
                        r = dr["rect"]
                        if r.x0 < ow and r.y1 < h_pg - 40 and r.y0 > 30:
                            ymax = max(ymax, r.y1)
                    y0 = min(ymax, h_pg - 40) + 10
                rect = pymupdf.Rect(46, y0, ow - 16, h_pg - 30)
                rc = -1
                if rect.height > 22:
                    rc = page.insert_textbox(rect, a["text"], fontname="japan",
                                             fontsize=7.3, color=RED_INK,
                                             lineheight=1.34, align=0)
                if rc >= 0:
                    body_cursors[a["page"]] = y0 + (rect.height - rc) + 6
                else:
                    a2 = dict(a)
                    a2["type"] = "comment"
                    resolved_extra.append((a2, rects))
